...
#             return f'''
#                 SELECT 
#                     CASE WHEN p = 'TRUE' THEN 1 ELSE 0 END AS count_sensor_node_1_tb,
#                     CASE WHEN p2 = 'TRUE' THEN 1 ELSE 0 END AS count_sensor_node_2_tb,
#                     CASE WHEN p3 = 'TRUE' THEN 1 ELSE 0 END AS count_sensor_node_3_tb,
#                     CASE WHEN p4 = 'TRUE' THEN 1 ELSE 0 END AS count_sensor_node_4_tb
#                 FROM 
#                     (
#                     SELECT
#                         CASE WHEN EXISTS (
#                             SELECT {table} 
#                             FROM `sensor_node_1_tb` 
#                             WHERE {table} >= (SELECT opt_temp_from FROM ideal_parameters) 
#                             AND {table} <= (SELECT opt_temp_to FROM ideal_parameters) AND CAST(Date_n_Time AS date) >= '2023-05-01' AND CAST(Date_n_Time AS date) <= '2023-05-31' 
#                         ) THEN 'TRUE' ELSE 'FALSE' 
#                         END AS p,
#                         CASE WHEN EXISTS (
#                             SELECT {table} 
#                             FROM `sensor_node_2_tb` 
#                             WHERE {table} >= (SELECT opt_temp_from FROM ideal_parameters) 
#                             AND {table} <= (SELECT opt_temp_to FROM ideal_parameters) AND CAST(Date_n_Time AS date) >= '2023-05-01' AND CAST(Date_n_Time AS date) <= '2023-05-31' 
#                         ) THEN 'TRUE' ELSE 'FALSE' 
#                         END AS p2,
